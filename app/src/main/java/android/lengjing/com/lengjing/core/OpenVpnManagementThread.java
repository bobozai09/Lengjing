...
public class OpenVpnManagementThread implements Runnable ,OpenVPNManagement{
    ...
    public void run() {
        ...
        try {
            ...
            while (true) {
                ...
                pendingInput += input;
                //Log.d("TAG","输出"+pendingInput);
                pendingInput = processInput(pendingInput);
                // Log.d("TAG","输出处理"+pendingInput);
            }
        } catch (IOException e) {
    ...
    private void processState(String argument) {
        String[] args = argument.split(",",6);
        String currentstate = args[1];
        // Log.d("TAG","抓取链接成功:"+args[1]+"---"+argument);
        if (args[2].equals(",,"))
            ...
            if(currentstate.equalsIgnoreCase("CONNECTED")){
                //链接成功后，存储
                Remember.putString(Constant.localIp,args[3]);
                Remember.putString(Constant.serverIp,args[4]);
            }
        }
    }
    ...
    private static boolean stopOpenVPN() {
        synchronized (active) {
            ...
            for (OpenVpnManagementThread mt : active) {
                ...
                count++;
                try {
                    Log.d("OpenVpn","--"+count+"--"+mt.mSocket.getSoTimeout());
                    if (mt.mSocket != null)
                        mt.mSocket.close();
                } catch (IOException e) {
                    Log.d("OpenVpn",e.getMessage());
